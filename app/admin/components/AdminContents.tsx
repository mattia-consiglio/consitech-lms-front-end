...
import type { Course, Lesson, PageableContent } from "@/utils/types"
...
export default function AdminContents({
	...
	const getCourses = useCallback(async () => {
		API.get<PageableContent<Course> | Course[]>(
			type === "course" ? "courses" : "courses/list",
		)
			.then((response) => {
				if (type === "course" && "content" in response)
		...
		<MainWrapper>
			...
			<div>
				...
				{!loading && !contents.length && (
					<p>Non ci sono {contentPluralWord}.</p>
				)}
				{!loading &&
					contents.length > 0 &&
					contents.map((course) => (

...
export default async function ServerCorsesComponent() {
	...
	const role = response && response !== true ? response.role : UserRole.USER

	let responseCourses: PageableContent<Course>

	if (token) {
		// L'utente è loggato, chiama l'endpoint protetto
		responseCourses = await API.get<PageableContent<Course>>("courses").catch(
			() => {
		...
		responseCourses = await API.get<PageableContent<Course>>(
			"public/courses",
		).catch(() => {
			return {} as PageableContent<Course>
		})
		...
		<>
			<div className="grid grid-cols-1 md:grid-cols-3 gap-x-2 gap-y-4 mt-4">
				<Suspense fallback={<div>Caricamento...</div>}>
					...
					{courses?.map((course) => (
						<CourseBlock key={course.id} role={role} {...course} />
					))}
				</Suspense>
			</div>
			{/* <CourseModal /> */}
